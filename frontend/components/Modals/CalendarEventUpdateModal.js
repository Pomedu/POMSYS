...
import { modalClose } from '../../store/modules/modalSlice';

const CalendarEventUpdateModal = (props) => {
    ...
    const onUpdate = (e) => {
        ...
        if (inputFields) {
            const changes = {
                start: inputFields.date+" "+inputFields.start_time,
                end: inputFields.date+" "+inputFields.end_time
            }
            console.log(changes);
            dispatch(updateLesson({ editedLesson: inputFields, lessonId: lessonData.id }))
            props.onUpdate({lessonId:lessonData.id, calendarId:lessonData.lecture.teacher, changes:changes})
        } else {
            console.log("생성못함");
        }
    };
...
export default CalendarEventUpdateModal