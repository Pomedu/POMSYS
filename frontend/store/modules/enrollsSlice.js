...
export const fetchTeacherEnrolls = createAsyncThunk("GET/TEACHER/ENROLL", async (teacherId, { rejectWithValue }) => {
    return axios({
        method: "get",
        url: `http://127.0.0.1:8000/api/teachers/${teacherId}/enrolls`,
    }).then(response => { return response.data })
        .catch(error => rejectWithValue(error.response.data));
});

export const fetchStudentEnrolls = createAsyncThunk("GET/STUDENT/ENROLL", async (studentId, { rejectWithValue }) => {
    return axios({
        method: "get",
        url: `http://127.0.0.1:8000/api/students/${studentId}/enrolls`,
    }).then(response => { return response.data })
        .catch(error => rejectWithValue(error.response.data));
});
...
export const enrollsSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(fetchTeacherEnrolls.pending, (state) => {
                state.error = null;
                state.loading = true;
            })
            .addCase(fetchTeacherEnrolls.fulfilled, (state, { payload }) => {
                state.loading = false;
                state.enrollsData = payload;
                state.filteredEnrollsData = payload;
            })
            .addCase(fetchTeacherEnrolls.rejected, (state, { payload }) => {
                state.error = payload;
                state.loading = false;
            })
            .addCase(fetchStudentEnrolls.pending, (state) => {
                state.error = null;
                state.loading = true;
            })
            .addCase(fetchStudentEnrolls.fulfilled, (state, { payload }) => {
                state.loading = false;
                state.enrollsData = payload;
                state.filteredEnrollsData = payload;
            })
            .addCase(fetchStudentEnrolls.rejected, (state, { payload }) => {
                state.error = payload;
                state.loading = false;
            })
